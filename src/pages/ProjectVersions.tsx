    ...
    mutationFn: async (sourceFilmId: string) => {
      ...
      if (chars?.length) {
        await supabase.from("characters").insert(
          chars.map(({ id, created_at, ...c }) => ({ ...c, film_id: newFilm.id }))
        );
      }
      ...
      if (shots?.length) {
        await supabase.from("shots").insert(
          shots.map(({ id, created_at, ...s }) => ({ ...s, film_id: newFilm.id }))
        );
      }

      // Copy script analyses
      ...
      if (analyses?.length) {
        await supabase.from("script_analyses").insert(
          analyses.map(({ id, created_at, updated_at, ...a }) => ({ ...a, film_id: newFilm.id }))
        );
      }
      ...
      const { data: safety } = await supabase
        ...
        .eq("film_id", sourceFilmId);
      if (safety?.length) {
        await supabase.from("content_safety").insert(
          safety.map(({ id, updated_at, ...s }) => ({ ...s, film_id: newFilm.id }))
        );
      }

      // Copy asset identity registry
      const { data: assets } = await supabase
        .from("asset_identity_registry")
        .select("*")
        .eq("film_id", sourceFilmId);
      if (assets?.length) {
        await supabase.from("asset_identity_registry").insert(
          assets.map(({ id, created_at, updated_at, ...a }) => ({ ...a, film_id: newFilm.id }))
        );
      }

      // Copy post production clips
      const { data: clips } = await supabase
        .from("post_production_clips")
        .select("*")
        .eq("film_id", sourceFilmId);
      if (clips?.length) {
        await supabase.from("post_production_clips").insert(
          clips.map(({ id, created_at, ...c }) => ({ ...c, film_id: newFilm.id }))
        );
      }
            ...
            <DialogContent>
              ...
              <div className="space-y-4 pt-2">
                ...
                <p className="text-xs text-muted-foreground">
                  This creates a blank version. Upload a new script in the Development phase.
                </p>
                <Button
